...
from    partido pa
        left join persona jugador1 on pa.jugador_1 = jugador1.id
        left join persona jugador2 on pa.jugador_2 = jugador2.id
        left join persona jugador3 on pa.jugador_3 = jugador3.id
        left join persona jugador4 on pa.jugador_4 = jugador4.id
        left join persona arbitro on pa.arbitro = arbitro.id
...
select  persona.nombre, p.fase_numero,
        IF(persona.id = g.ganadorid, t.premio_single_masculino, premio_consuelo) ganancias,
        impto(IF(persona.id = g.ganadorid, t.premio_single_masculino, premio_consuelo)) impto,
        IF(persona.id = g.ganadorid, t.premio_single_masculino, premio_consuelo) - impto(IF(persona.id = g.ganadorid, t.premio_single_masculino, premio_consuelo)) liquido
from    persona
...
where   persona.id = 3 and f.torneo_ano = 1979 and f.estadio_id = 1
 ...
 */
 ...
 select partido.id, entrenador_1, e1.nombre, entrenador_2, e2.nombre, fecha
 from   partido
        inner join persona e1 on partido.entrenador_1 = e1.id
        inner join persona e2 on partido.entrenador_2 = e2.id;
